...
@pytest.mark.asyncio
async def test_admin_popular_keywords_apply_popular_keyword_overlay():
    """관리자 인기 검색어도 display_rank/manual_priority/is_excluded 규칙으로 재정렬됩니다."""
    service = TrendingService(conn=MagicMock(), redis_client=MagicMock())
    now = datetime.now(timezone.utc)
    ...
    service._history_repo.get_keyword_stats_since = AsyncMock(return_value={
        "듄": {"search_count": 0, "click_count": 0},
        "인터스텔라": {"search_count": 10, "click_count": 4},
    })

    response = await service.get_admin_popular_keywords(period="36500d", limit=2)

    assert [item.keyword for item in response.keywords] == ["듄", "인터스텔라"]
    assert response.keywords[0].search_count == 0
    assert response.keywords[0].conversion_rate == 0.0
    assert response.keywords[1].search_count == 30
    assert response.keywords[1].conversion_rate == 0.4


def test_popular_search_keyword_dto_normalizes_mysql_bit_flag():
    """popular_search_keyword.is_excluded BIT(1) bytes 값을 bool로 정규화합니다."""
    dto = PopularSearchKeywordDTO(
        id=1,
        keyword="듄",
        display_rank=1,
        manual_priority=9999,
    ...
    assert dto.is_excluded is True
